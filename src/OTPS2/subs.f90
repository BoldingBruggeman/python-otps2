...
!                  MS4,MN4,M6,M8,MK3,S6,2SM2,2MK3,2q1,m3
!
	data index/30,35,19,12,27,17,37,10,25,26,28,33,34, &
          23,14,24,11,5,3,2,45,46,44,50,0,42,51,40,0,18,8,41/
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
! Lana Erofeeva, June 2004, modified Oct 2021
! to exclude inferrence of minor constituents
! already in given model
...
!         -----------------
         ...
         real*8 u(18),f(18),arg(18),msk(18)
...
! re-order to correspond to cid8
         ...
         do i=1,8
          ...
          enddo
         enddo
! find constituent to turn inference off
         do i=1,18
           do j=1,ncon
            if(lower_case(cid(j)).eq.cmin(i))msk(i)=0;
           enddo
           !if(msk(i).eq.0)write(*,*)'No inference for ',cmin(i)
         enddo
...
!     ------------------
        dh = 0.D0
        do i=1,18
         dh = dh + msk(i)*dreal(zmin(i))*f(i)*cos((arg(i)+u(i))*rad)- &
                   msk(i)*dimag(zmin(i))*f(i)*sin((arg(i)+u(i))*rad)
        enddo
        return
        end
...
! this function replaces upper case letters with lower case
! in constituent names (ONLY those mentioned in constit.h!)
        character*4 function lower_case(c)
        character*4 c,t
! letters are in correspondence with constit.h names
        character*1 letc(15),let(15)
        integer(kind=4) k,j 
        data letc/'M','S','K','O','N','P','Q', &
                  'U','L','T','J','R','H','F','A'/
        data let/'m','s','k','o','n','p','q', &
                 'u','l','t','j','r','h','f','a'/
!
        t=c
        do k=1,15
         do j=1,4
         if(t(j:j).eq.letc(k))t(j:j)=let(k)
         enddo
        enddo
        lower_case=t
        return
